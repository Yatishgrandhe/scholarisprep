#!/usr/bin/env node
/**
 * Import all batch SQL files via Supabase MCP execute_sql (stdio).
...
const to = Number(process.env.TO ?? 98);
...
async function main() {
  ...
  send(proc, "initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "scholaris-import-v3", version: "1.0.0" },
  });
  ...
  for (let i = from; i <= to; i++) {
    ...
    try {
      ...
      if (result?.isError === true) throw new Error(text.slice(0, 500));
      successes++;
  ...
  proc.kill();
  console.log(JSON.stringify({ successes, failures, verify }, null, 2));
}
