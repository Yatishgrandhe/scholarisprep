#!/usr/bin/env node
/**
 * Apply .hard-mega-payloads/mega-XX.json via Supabase MCP execute_sql.
 * Usage: node scripts/apply-hard-mega-mcp.mjs [payloadDir] [start] [end] [delayMs]
 */
...
async function mcpCall(toolName, args) {
  ...
  return new Promise((resolve, reject) => {
    ...
    send("initialize", {
      ...
      capabilities: {},
      clientInfo: { name: "scholaris-apply-hard-mega", version: "1.0.0" },
    });
...
const files = readdirSync(payloadDir)
  .filter((f) => /^mega-\d+\.json$/.test(f))
  .sort((a, b) => Number(a.match(/mega-(\d+)/)[1]) - Number(b.match(/mega-(\d+)/)[1]))
  .filter((f) => {
    const n = Number(f.match(/mega-(\d+)/)[1]);
...
logLine(`# hard-mega apply ${new Date().toISOString()} files=${files.length}`);
