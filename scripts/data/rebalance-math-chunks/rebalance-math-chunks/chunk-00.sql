BEGIN;
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.451Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '0ddf3470-b3bc-4d1e-b114-b151e5702c79';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '0df3c9a4-0219-4c62-9786-5f5c04ca552e';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '158a18e8-7e24-45f4-b163-776aa9da987a';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '2c1f9227-3149-49a1-b08b-a12ca7a63794';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '32d62b4c-2965-4091-b684-c5331bbfcea4';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '3387e0ba-4ca6-4fa1-97c4-1d4f40879388';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '36a98104-8f91-4dd8-880a-46168d3621db';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '46e59d1a-e924-4e97-860d-2691a62828f1';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '4ed1bc33-4c0a-4854-afcf-d83bf8b435ed';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '52fda01c-320a-4123-bad2-4846bac22b66';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '5616597f-96c7-422f-bc2e-b61d6ba54222';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '5a76dd3e-59d1-4db9-bad3-415f766b63bf';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '5d9114c3-042c-4088-a2cb-5de8849bada2';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '5fa6e580-3eff-487b-a1a7-68a044eee3f9';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '60dab1d0-93f9-49c5-b960-efdd249064e8';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '64bcb47c-ba98-4330-9918-8fade2b7158c';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '6983a6c4-7898-425e-91b2-22a7ea35ea55';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '70ed7eda-6626-4b16-ae65-6de622f2b289';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '7557d701-ac40-4406-b2b1-d70d2bf0e009';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '00311fb1-3401-46f0-95fb-4a4d15ad7064';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '01c1c54b-a970-4001-9408-d60934a95c92';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '04b7d39f-28c3-42c7-99bf-a72133005fb6';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '0b5f8049-e9db-4e25-85de-2e04c78e4627';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '0bdc95fe-95d6-4f56-ab10-7f80b58d1d4c';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '11259cf9-3cfe-43d9-9870-25117beced80';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '12e12293-d795-445a-892d-3afb1119332f';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '1592559f-5410-4b03-ac82-438a8ccaa1b0';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '15ac1fc1-a2c2-4814-b7b2-8f6fbbc54f77';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '172ec524-c6e5-4e62-9e46-c608972c6f15';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '18ca3b23-4407-4d45-b84f-8fc34b6b4da9';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '1a200755-6a2c-4eb6-89c5-68ea885ceff7';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '1aa81f0d-8829-4063-aa14-e2db7e9b9295';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '1bd4cf9b-5130-4fa8-98e9-dee0b6ae50e5';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '1bdf891e-a8de-4ab8-835b-25b4b3ed06d7';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '1da68665-dd04-4b6e-9c69-088b78e857cc';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '2205b3bb-c342-4101-a631-b8f4ca282593';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '224eeb9a-24a0-4d0a-ab82-cd508dec47b0';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '2266b9a2-d2c6-475f-8e6e-450c65727d46';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '23008117-8a9f-4499-a89c-76ffeae1c658';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '23c3717d-60f8-4857-a79a-08091e499f83';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '27d3b8f7-5263-491d-987b-9cf722ddcbcc';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '27e49ed8-bf9b-4fbd-8fab-72ad0eda67ad';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '2d9f06e5-1fcb-4671-862f-b43ff257ce44';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '3505aba1-fabf-4c4c-b8a5-8d5376f20891';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '35d248dc-ef7b-4afa-9a79-cd727c2db4ae';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '3b6e9dfb-2f9a-4eac-84e1-e57ac042134e';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '3b7d055f-becf-4a6c-a251-aaa1824cc133';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '3d756a25-815a-4a3c-b59e-c8f6822f5437';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '46c413f3-ec3f-4ffd-8648-8379225e0ad1';
UPDATE public.questions SET
  difficulty = 'easy',
  source_metadata = COALESCE(source_metadata, '{}'::jsonb) || '{"math_difficulty_rebalance":"math_difficulty_rebalance_v1","math_difficulty_rebalanced_at":"2026-06-12T21:40:02.455Z","rebalance_method":"rebalance-math-difficulty.mjs","prior_difficulty":"medium","rebalance_from":"medium","rebalance_to":"easy"}'::jsonb,
  updated_at = now()
WHERE id = '4767d953-d1dc-44f0-a64a-477c74f3a517';
COMMIT;
